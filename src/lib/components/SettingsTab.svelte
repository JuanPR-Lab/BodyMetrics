<script lang="ts">
    ...
    import { Users, BarChart3, Save, Download, Upload, AlertTriangle, Trash2, Scale } from 'lucide-svelte';
    ...
    // Para evitar el Error 500 (SSR), inicializamos en 0 y calculamos al montar
    let totalMeasurements = 0;
    let isMounted = false;

    // Reactividad para el contador (solo si estamos en el navegador)
    ...
    const handleBackupFile = async (file: File) => {
        ...
        try {
            ...
            if (success) {
                ...
                // Forzamos actualización del contador tras importar
                totalMeasurements = PatientManager.getAssignmentCount();
            } else {
    ...
    <div class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden mb-6">
        <div class="bg-slate-50 px-6 py-4 border-b border-slate-100">
            <h3 class="font-bold text-slate-700 flex items-center gap-2">
                ...
                {$t('settings.display_section')}
            </h3>
        </div>

        <div class="p-6">
            <span class="text-xs font-bold text-slate-400 uppercase tracking-wider mb-3 block">
                {$t('settings.unit_preference')}
            </span>
            
            <div class="grid grid-cols-3 gap-2 p-1 bg-slate-100 rounded-xl">
    			{#each ['kg', 'lb', 'st-lb'] as u}
        			<button
            			type="button"
            			onclick={() => settings.setUnit(u as any)}
            			class="py-2.5 rounded-lg font-bold text-sm transition-all {settings.unit === u 
