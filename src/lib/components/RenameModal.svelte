<script lang="ts">
	...
	import { Edit3, AlertCircle } from 'lucide-svelte';
	...
	// New props to handle error state - when true, modal stays open after confirm
	...
	$: if (isOpen) {
		inputValue = '';
		hasError = false; // Reset error state when modal opens
	}

	// Auto-focus the input when modal opens or when there's an error
	$: if (isOpen || hasError) {
		setTimeout(() => {
			inputElement?.focus();
			inputElement?.select();
	...
	function close() {
		dispatch('close');
		// Always allow closing, but parent will decide whether to actually close based on error state
	}
	...
	function confirm() {
		...
		// If hasError is true, the modal will stay open and input will keep focus
	}
		...
		>
			...
			<div class="px-6 py-6">
				<div class="space-y-2">
					<input
						...
						class="w-full border-2 border-slate-200 rounded-lg px-3 py-2 text-slate-800 focus:border-indigo-500 focus:ring-indigo-500 outline-none transition-all {hasError
							? 'border-red-500'
							: ''}"
						autocomplete="off"
					/>
					{#if hasError}
						<p class="text-red-500 text-sm mt-1 flex items-center gap-1">
							...
							{errorMessage || $t('dashboard.client_exists_message')}
						</p>
					{/if}
				</div>
			</div>
			...
			<div class="px-6 py-4 bg-gray-50 border-t border-gray-100 flex justify-end gap-3">
				...
				<button
					...
					class="px-4 py-2 bg-indigo-600 text-white font-bold text-sm rounded-lg shadow-sm flex items-center gap-2 transition-all {inputValue.trim()
