<script lang="ts">
    ...
    // NUEVAS IMPORTACIONES PARA UNIDADES
    ...
    // --- LÓGICA DE DISEÑO ADAPTATIVO (Corrección para st-lb) ---
    $: isStone = settings.unit === 'st-lb';
    $: dynamicFontSize = isStone ? '12' : '18';
    $: dynamicBoxWidth = isStone ? '100' : '85';
    $: textCenterX = isStone ? 50 : 42.5;
    ...
    $: data = record
        ? {
                fat: {
                    armR: record.fatArmR,
                    armL: record.fatArmL,
                    trunk: record.fatTrunk,
                    legR: record.fatLegR,
                    legL: record.fatLegL,
                    unit: 'percent' // Usamos clave para i18n (%)
                },
                muscle: {
                    armR: record.muscleArmR,
                    armL: record.muscleArmL,
                    trunk: record.muscleTrunk,
                    legR: record.muscleLegR,
                    legL: record.muscleLegL,
                    unit: settings.unit // Dinámico: kg, lb o st-lb
                }
            }
        : null;
    ...
    const getTrafficLightClass = (blockType: HealthStatus, activeStatus: string): string => {
        ...
        switch (blockType) {
            case 'under':
                return isActive
                    ? 'bg-blue-500 ring-2 ring-blue-400 scale-110 opacity-100 shadow-lg'
                    : 'bg-blue-200 opacity-30';
            case 'healthy':
                return isActive
                    ? 'bg-green-500 ring-2 ring-green-400 scale-110 opacity-100 shadow-lg'
                    : 'bg-green-200 opacity-30';
            case 'over':
                return isActive
                    ? 'bg-yellow-400 ring-2 ring-yellow-300 scale-110 opacity-100 shadow-lg'
                    : 'bg-yellow-100 opacity-30';
            case 'obese':
                return isActive
                    ? 'bg-red-500 ring-2 ring-red-400 scale-110 opacity-100 shadow-lg'
                    : 'bg-red-200 opacity-30';
            default:
                return '';
        }
    };
...
<div
    class="bg-white p-3 sm:p-4 rounded-xl border border-gray-200 shadow-sm flex flex-col h-full relative"
>
    <div
        class="flex flex-col sm:flex-row justify-between items-center mb-2 z-10 border-b border-gray-100 pb-2"
    >
        ...
        <div
            class="flex bg-gray-50 p-1 rounded-lg border border-gray-200 shadow-inner"
            role="group"
        ...
        >
            <button
                ...
                class="text-[10px] font-bold px-3 sm:px-4 py-1.5 sm:py-2 rounded-md transition-all uppercase tracking-wide {viewMode ===
                'fat'
    ...
    <div
        class="flex-1 flex flex-col items-center justify-center relative min-h-[300px] sm:min-h-[400px] w-full"
    >
        {#if !record}
        ...
        {:else}
            <svg
                viewBox="-50 0 380 520"
                class="h-full w-full max-h-[400px] sm:max-h-[600px] drop-shadow-xl overflow-visible"
            ...
            >
                ...
                <path
                    d="M140,20 C128,20 120,32 120,50 C120,72 128,82 140,82 C152,82 160,72 160,50 C160,32 152,20 140,20 Z"
                    fill="#e5e7eb"
                />
                ...
                <g
                    ...
                    style={hoveredSegment === 'armR'
                        ? 'transform: scale(1.02); filter: url(#hover-glow);'
                        : ''}
                >
                    <path
                        ...
                        style="fill: {hoveredSegment === 'armR'
                            ? hoverFill
                            : activeFill}; stroke: white; stroke-width: 3px;"
                        class="transition-colors duration-300"
                    />
                    <g transform="translate({isStone ? -60 : -45}, 130)">
                        <line
                            x1={dynamicBoxWidth}
                            y1="25"
                            x2={isStone ? 120 : 105}
                        ...
                        <text
                            ...
                            fill="#64748b"
                            text-anchor="middle"
                            class="uppercase tracking-wider">{$t('segments.right_arm')}</text
                        >
                        <text
                            x={textCenterX}
                            y="40"
                            ...
                            text-anchor="middle"
                        >
                            {viewMode === 'muscle' ? formatWeight(currentData?.armR, settings.unit) : formatVal(currentData?.armR)}
                            <tspan font-size="9" font-weight="bold" fill="#94a3b8" dy="0" dx="1"
                                >{$t('units.' + currentData?.unit)}</tspan
                            >
                        </text>
                    </g>
                </g>
                ...
                >
                    <path
                        ...
                        style="fill: {hoveredSegment === 'armL'
                            ? hoverFill
                            : activeFill}; stroke: white; stroke-width: 3px;"
                        class="transition-colors duration-300"
                    />
                    <g transform="translate(235, 130)">
                        ...
                        <text
                            ...
                            fill="#64748b"
                            text-anchor="middle"
                            class="uppercase tracking-wider">{$t('segments.left_arm')}</text
                        >
                        <text
                            x={textCenterX}
                            y="40"
                            ...
                            text-anchor="middle"
                        >
                            {viewMode === 'muscle' ? formatWeight(currentData?.armL, settings.unit) : formatVal(currentData?.armL)}
                            <tspan font-size="9" font-weight="bold" fill="#94a3b8" dy="0" dx="1"
                                >{$t('units.' + currentData?.unit)}</tspan
                            >
                        </text>
                ...
                >
                    <path
                        ...
                        style="fill: {hoveredSegment === 'trunk'
                            ? hoverFill
                            : activeFill}; stroke: white; stroke-width: 3px;"
                        class="transition-colors duration-300"
                    />
                    <g transform="translate({isStone ? 92 : 100}, 115)">
                        <rect
                            x="0"
                            y="0"
                            width={dynamicBoxWidth}
                        ...
                        <text
                            ...
                            fill="#64748b"
                            text-anchor="middle"
                            class="uppercase tracking-wider">{$t('segments.trunk')}</text
                        >
                        <text
                            x={textCenterX}
                            y="36"
                            ...
                            text-anchor="middle"
                        >
                            {viewMode === 'muscle' ? formatWeight(currentData?.trunk, settings.unit) : formatVal(currentData?.trunk)}
                            <tspan font-size="9" font-weight="bold" fill="#94a3b8" dy="0" dx="1"
                                >{$t('units.' + currentData?.unit)}</tspan
                            >
                        </text>
                ...
                >
                    ...
                    <g transform="translate({isStone ? -5 : 10}, 310)">
                        ...
                        <text
                            ...
                            fill="#64748b"
                            text-anchor="middle"
                            class="uppercase tracking-wider">{$t('segments.right_leg')}</text
                        >
                        <text
                            x={textCenterX}
                            y="36"
                            ...
                            text-anchor="middle"
                        >
                            {viewMode === 'muscle' ? formatWeight(currentData?.legR, settings.unit) : formatVal(currentData?.legR)}
                            <tspan font-size="9" font-weight="bold" fill="#94a3b8" dy="0" dx="1"
                                >{$t('units.' + currentData?.unit)}</tspan
                            >
                        </text>
                ...
                >
                    ...
                    <g transform="translate(190, 310)">
                        ...
                        <text
                            ...
                            fill="#64748b"
                            text-anchor="middle"
                            class="uppercase tracking-wider">{$t('segments.left_leg')}</text
                        >
                        <text
                            x={textCenterX}
                            y="36"
                            ...
                            text-anchor="middle"
                        >
                            {viewMode === 'muscle' ? formatWeight(currentData?.legL, settings.unit) : formatVal(currentData?.legL)}
                            <tspan font-size="9" font-weight="bold" fill="#94a3b8" dy="0" dx="1"
                                >{$t('units.' + currentData?.unit)}</tspan
                            >
                        </text>
            ...
            {#if viewMode === 'fat'}
                ...
                >
                    <div class="flex justify-between items-end h-3 mb-1 px-1">
                        <div
                            class="w-3 h-3 border-l-2 border-t-2 border-gray-600 bg-white rotate-45 transform transition-all duration-300
              ...
              "
                    ...
                    <div
                        class="flex w-full h-3 rounded-full overflow-hidden shadow-inner bg-gray-100 gap-[2px]"
                    >
                        <div
                            class="flex-1 transition-all duration-300 {getTrafficLightClass(
                                'under',
                                currentStatus
                            )}"
                        ></div>
