<script lang="ts">
  ...
  // ICONOS
  import {
    Users, ChevronDown, ChevronLeft, ChevronRight, CheckCircle,
    FileSpreadsheet, Edit, Trash2, AlertCircle, Undo2,
    Scale, Activity, Droplets, Dumbbell, Bone, Flame, Clock,
    BarChart3, Info, Download, Monitor
  } from 'lucide-svelte';
  ...
  $: clientHistory = effectiveReadOnly && records
    ? records
    : (currentClient
  ...
  $: currentRecord = selectedRecordId
    ...
    : (displayedHistory.length > 0 ? displayedHistory[0] : null);

  $: chartData = (currentClient && displayedHistory.length > 0)
    ? PatientManager.computeChartData(displayedHistory, selectedChartMetric as keyof BioMetricRecord)
    : null;

  $: activeChartColor = CHART_OPTIONS.find(o => o.key === selectedChartMetric)?.color || '#6366f1';
  ...
  // CORREGIDO: Clave directa metrics_info (sin dashboard.)
  ...
  // Botón Guardar (Versión Híbrida Segura)
  ...
  function handleCreateClient(e?: Event) {
    ...
    // CAMBIO AQUÍ: Comenta o borra esta línea para que NO se abra la lista
  ...
  function handleDeleteClient() { dispatch('deleteClient', selectedClientId); }
  function handleRenameClient() { dispatch('renameClient', { id: selectedClientId, newName: currentClient?.alias || '' }); }
  function handleUnassignRecord() {
      if (currentRecord) {
        // CAMBIO: Emitimos 'requestUnassign' en lugar de 'unassignRecord'
        // Esto le dice al padre: "Oye, quiero borrar esto, pregunta al usuario si está seguro"
        dispatch('requestUnassign', currentRecord.id); 
      }
  }
  function handleExport() { dispatch('exportClient', selectedClientId); }
  ...
  const STYLES = {
    ...
    filterBtnInactive: "bg-white text-slate-500 border-slate-200 hover:border-indigo-300 hover:text-indigo-600"
  };
</script>
...
<div class="flex flex-col lg:grid lg:grid-cols-4 gap-4 sm:gap-6 h-auto lg:h-[800px] animate-fade-in">
  ...
  <div class="lg:col-span-1 flex flex-col gap-3 sm:gap-4 h-auto lg:h-full">
    ...
    <div class="bg-indigo-50 p-3 sm:p-4 rounded-lg shadow-sm border border-indigo-100 flex-shrink-0 relative z-0">
    ...
    <div class="lg:hidden bg-white rounded-lg shadow-sm border border-gray-200 flex-shrink-0">
      ...
      >
          ...
          <ChevronDown class="text-gray-400 absolute right-3 top-1/2 -translate-y-1/2 transition-transform {isClientListOpen ? 'rotate-180' : ''}" size={16} />
      </button>
      ...
      {#if isClientListOpen}
        <div class="border-t border-gray-100">
          ...
          <div class="max-h-[300px] overflow-y-auto p-1 sm:p-2 space-y-1">
             {#if clients.length === 0}
               <div class="p-4 text-center text-xs text-slate-400 italic">{$t('dashboard.no_clients_created')}</div>
             {:else}
                {#each paginatedClients as client (client.id)}
                  ...
                  >
                    ...
                    <span class="text-[10px] bg-indigo-100 text-indigo-700 px-2 py-1 rounded-full font-bold min-w-[24px] text-center">{clientCounts[client.id] || 0}</span>
                  </button>
                {/each}
             {/if}
          </div>
          {#if filteredClients.length > clientsPerPage}
             <div class="border-t border-gray-100 p-2 bg-gray-50 flex justify-center gap-4 items-center">
                <button on:click={() => currentPage = Math.max(1, currentPage - 1)} disabled={currentPage === 1} class="p-1 rounded border bg-white hover:bg-gray-50 disabled:opacity-50"><ChevronLeft size={16}/></button>
                <span class="text-xs text-gray-500">{$t('dashboard.pagination.page_of', { values: { current: currentPage, total: totalPages } })}</span>
                <button on:click={() => currentPage = Math.min(totalPages, currentPage + 1)} disabled={currentPage === totalPages} class="p-1 rounded border bg-white hover:bg-gray-50 disabled:opacity-50"><ChevronRight size={16}/></button>
            </div>
          {/if}
        </div>
      {/if}
    ...
    <div class="hidden lg:flex bg-white rounded-lg shadow-sm border border-gray-200 flex-1 overflow-hidden flex-col h-[560px]">
      <div class="p-2 border-b bg-gray-50">
        <input type="text" bind:value={clientSearchTerm} placeholder={$t('dashboard.filter_placeholder')} class="w-full text-sm border rounded px-3 py-2 bg-white focus:ring-1 focus:ring-indigo-500 outline-none" />
      </div>
      ...
      <div class="overflow-y-auto flex-1 p-1 sm:p-2 space-y-1">
        {#if clients.length === 0}
           <div class="flex flex-col items-center justify-center h-32 text-center p-4">
              ...
              <p class="text-xs text-slate-400 italic leading-relaxed">{$t('dashboard.no_clients_created')}</p>
           </div>
        {:else}
            {#each paginatedClients as client (client.id)}
      ...
      {#if filteredClients.length > clientsPerPage}
        <div class="border-t border-gray-200 p-2 bg-gray-50 flex justify-center gap-4 items-center mt-auto">
      ...
      <div class="bg-white p-4 rounded-xl shadow-sm border border-gray-200 flex flex-col gap-4 flex-shrink-0">
        ...
        <div class="flex flex-col sm:flex-row items-center sm:justify-between gap-3 sm:gap-4 w-full">
          <div class="text-center sm:text-left">
            <h2 class="text-2xl font-bold text-gray-800 break-words">{effectiveReadOnly && clientName ? clientName : currentClient?.alias}</h2>
          </div>

          <div class="grid grid-cols-1 sm:grid-cols-3 gap-2 w-full sm:w-auto">
            
            <button on:click={handleExport} class="w-full sm:col-span-1 justify-center text-emerald-600 hover:text-white border border-emerald-200 hover:bg-emerald-600 text-xs font-bold px-3 py-1.5 rounded-lg transition-colors shadow-sm flex items-center gap-2" title="{$t('dashboard.export_csv_btn')}">
              <FileSpreadsheet size={14} /><span>{$t('dashboard.export_csv_btn')}</span>
            </button>

            {#if !effectiveReadOnly}
              <button on:click={handleRenameClient} class="w-full sm:col-span-1 sm:w-auto justify-center text-indigo-600 hover:text-white border border-indigo-200 hover:bg-indigo-600 text-xs font-bold px-3 py-1.5 rounded-lg transition-colors shadow-sm flex items-center gap-2" title="{$t('actions.rename')}">
                <Edit size={14} /><span>{$t('actions.rename')}</span>
              </button>

              <button on:click={handleDeleteClient} class="w-full sm:col-span-1 sm:w-auto justify-center text-red-600 hover:text-white border border-red-200 hover:bg-red-600 text-xs font-bold px-3 py-1.5 rounded-lg transition-colors shadow-sm flex items-center gap-2" title="{$t('actions.delete')}">
        ...
        <div class="flex flex-col sm:flex-row items-center gap-3 border-t pt-3 w-full">
           <div class="flex flex-col sm:flex-row items-center sm:items-start w-full sm:w-auto gap-2 sm:gap-0">
               ...
               <div class="flex flex-wrap justify-center sm:justify-start gap-2 flex-grow px-10 sm:px-0">
                  <button on:click={() => currentFilter = 'all'} class="{STYLES.filterBtn} {currentFilter === 'all' ? STYLES.filterBtnActive : STYLES.filterBtnInactive}">{$t('dashboard.filters.all')}</button>
                  <button on:click={() => currentFilter = '1m'} class="{STYLES.filterBtn} {currentFilter === '1m' ? STYLES.filterBtnActive : STYLES.filterBtnInactive}">{$t('dashboard.filters.last_month')}</button>
                  <button on:click={() => currentFilter = '3m'} class="{STYLES.filterBtn} {currentFilter === '3m' ? STYLES.filterBtnActive : STYLES.filterBtnInactive}">{$t('dashboard.filters.last_3_months')}</button>
                  <button on:click={() => currentFilter = '6m'} class="{STYLES.filterBtn} {currentFilter === '6m' ? STYLES.filterBtnActive : STYLES.filterBtnInactive}">{$t('dashboard.filters.last_6_months')}</button>
                  <button on:click={() => currentFilter = '1y'} class="{STYLES.filterBtn} {currentFilter === '1y' ? STYLES.filterBtnActive : STYLES.filterBtnInactive}">{$t('dashboard.filters.last_year')}</button>
               </div>
           </div>
           ...
           <div class="flex items-center gap-2 sm:ml-auto border border-slate-200 rounded-xl px-4 py-2 bg-white flex-shrink-0 mt-2 sm:mt-0 shadow-sm transition-colors hover:border-indigo-300">
             ...
             <input type="date" bind:value={customDateStart} on:change={() => currentFilter = 'custom'} class="text-xs text-gray-500 font-bold bg-transparent outline-none cursor-pointer hover:text-indigo-600 transition-colors" />
             <span class="text-slate-300 mx-1">|</span>
             <span class="text-[10px] font-bold text-slate-400 uppercase tracking-wider">{$t('dashboard.filters.to')}</span>
             <input type="date" bind:value={customDateEnd} on:change={() => currentFilter = 'custom'} class="text-xs text-gray-500 font-bold bg-transparent outline-none cursor-pointer hover:text-indigo-600 transition-colors" />
           </div>
        </div>
      </div>
      ...
      <div class="w-full flex-shrink-0 bg-gray-50 p-2 sm:p-3 rounded-xl border border-gray-200 shadow-inner overflow-x-auto scrollbar-thin">
        {#if displayedHistory.length === 0}
            <div class="flex flex-col items-center justify-center py-4 text-center px-4 w-full h-[85px] sm:h-[105px]">
                ...
                {:else}
                    {#if (clientCounts[currentClient?.id || ''] || 0) === 0}
                        <div class="text-gray-400 flex flex-col items-center gap-1">
                            <span class="text-xs italic">{$t('dashboard.client_no_history')}</span>
                        </div>
                    {:else}
        ...
        <div class="grid grid-cols-1 xl:grid-cols-2 gap-4 sm:gap-6">
            <div class="flex flex-col gap-3 sm:gap-4 xl:col-span-1">
                <div class="grid grid-cols-2 gap-3 sm:gap-4">
                    <div class="bg-white p-3 sm:p-4 rounded-xl shadow-sm border border-gray-100 border-l-4 border-l-slate-800 transition-transform hover:scale-[1.02] h-full flex flex-col justify-between">
                        <div class="min-h-[24px] flex items-center justify-between mb-2"><span class="text-[10px] sm:text-xs font-black text-slate-700 uppercase tracking-widest truncate">{$t('metrics.weight')}</span></div>
                        <div class="flex items-end justify-between mt-1"><Scale size={20} class="text-slate-400 mb-1" strokeWidth={2} /><div class="text-right leading-none"><span class="text-xl sm:text-2xl font-black text-slate-800">{currentRecord?.weight ?? '--'}</span><span class="text-[10px] sm:text-xs font-bold text-slate-400 ml-0.5">{$t('units.kg')}</span></div></div>
                    </div>
                    <div class="bg-white p-3 sm:p-4 rounded-xl shadow-sm border border-gray-100 border-l-4 border-l-pink-500 transition-transform hover:scale-[1.02] h-full flex flex-col justify-between">
                        <div class="min-h-[24px] flex items-center justify-between mb-2"><span class="text-[10px] sm:text-xs font-black text-slate-700 uppercase tracking-widest truncate">{$t('metrics.bmi')}</span><button on:click={() => openInfo('bmi')} class="text-slate-300 hover:text-indigo-500 transition-colors"><Info size={16} /></button></div>
                        <div class="flex items-end justify-between mt-1"><Activity size={20} class="text-pink-500 mb-1" strokeWidth={2} /><span class="text-xl sm:text-2xl font-black leading-none {getStatusColor('bmi', currentRecord?.bmi, currentRecord).replace('bg-', 'text-').replace('-100', '-600')}">{currentRecord?.bmi ?? '--'}</span></div>
                    </div>
                    <div class="bg-white p-3 sm:p-4 rounded-xl shadow-sm border border-gray-100 border-l-4 border-l-amber-500 transition-transform hover:scale-[1.02] h-full flex flex-col justify-between">
                        <div class="min-h-[24px] flex items-center justify-between mb-2"><span class="text-[10px] sm:text-xs font-black text-slate-700 uppercase tracking-widest truncate">{$t('metrics.body_fat')}</span><button on:click={() => openInfo('body_fat')} class="text-slate-300 hover:text-indigo-500 transition-colors"><Info size={16} /></button></div>
                        <div class="flex items-end justify-between mt-1"><Droplets size={20} class="text-amber-400 mb-1" strokeWidth={2} /><div class="text-right leading-none"><span class="text-xl sm:text-2xl font-black {getStatusColor('fat', currentRecord?.bodyFat, currentRecord).replace('bg-', 'text-').replace('-100', '-600')}">{currentRecord?.bodyFat ?? '--'}</span><span class="text-[10px] sm:text-xs font-bold text-slate-400 ml-0.5">%</span></div></div>
                    </div>
                    <div class="bg-white p-3 sm:p-4 rounded-xl shadow-sm border border-gray-100 border-l-4 border-l-indigo-500 transition-transform hover:scale-[1.02] h-full flex flex-col justify-between">
                        <div class="min-h-[24px] flex items-center justify-between mb-2"><span class="text-[10px] sm:text-xs font-black text-slate-700 uppercase tracking-widest truncate">{$t('metrics.muscle_mass')}</span></div>
                        <div class="flex items-end justify-between mt-1"><Dumbbell size={20} class="text-indigo-400 mb-1" strokeWidth={2} /><div class="text-right leading-none"><span class="text-xl sm:text-2xl font-black text-slate-800">{currentRecord?.muscleMass ?? '--'}</span><span class="text-[10px] sm:text-xs font-bold text-slate-400 ml-0.5">kg</span></div></div>
                    </div>
                    <div class="bg-white p-3 sm:p-4 rounded-xl shadow-sm border border-gray-100 border-l-4 border-l-cyan-500 transition-transform hover:scale-[1.02] h-full flex flex-col justify-between">
                        <div class="min-h-[24px] flex items-center justify-between mb-2"><span class="text-[10px] sm:text-xs font-black text-slate-700 uppercase tracking-widest truncate">{$t('metrics.water')}</span><button on:click={() => openInfo('water')} class="text-slate-300 hover:text-indigo-500 transition-colors"><Info size={16} /></button></div>
                        <div class="flex items-end justify-between mt-1"><Droplets size={20} class="text-cyan-400 mb-1" strokeWidth={2} /><div class="text-right leading-none"><span class="text-xl sm:text-2xl font-black text-slate-800">{currentRecord?.waterPercentage ?? '--'}</span><span class="text-[10px] sm:text-xs font-bold text-slate-400 ml-0.5">%</span></div></div>
                    </div>
                    <div class="bg-white p-3 sm:p-4 rounded-xl shadow-sm border border-gray-100 border-l-4 border-l-gray-400 transition-transform hover:scale-[1.02] h-full flex flex-col justify-between">
                        <div class="min-h-[24px] flex items-center justify-between mb-2"><span class="text-[10px] sm:text-xs font-black text-slate-700 uppercase tracking-widest truncate">{$t('metrics.bone_mass')}</span></div>
                        <div class="flex items-end justify-between mt-1"><Bone size={20} class="text-slate-400 mb-1" strokeWidth={2} /><div class="text-right leading-none"><span class="text-xl sm:text-2xl font-black text-slate-800">{currentRecord?.boneMass ?? '--'}</span><span class="text-[10px] sm:text-xs font-bold text-slate-400 ml-0.5">kg</span></div></div>
                    </div>
                    <div class="bg-white p-3 sm:p-4 rounded-xl shadow-sm border border-gray-100 border-l-4 border-l-emerald-500 transition-transform hover:scale-[1.02] h-full flex flex-col justify-between">
                        <div class="min-h-[24px] flex items-center justify-between mb-2"><span class="text-[10px] sm:text-xs font-black text-slate-700 uppercase tracking-widest truncate">{$t('metrics.dci')}</span></div>
                        <div class="flex items-end justify-between mt-1"><Flame size={20} class="text-emerald-400 mb-1" strokeWidth={2} /><div class="text-right leading-none"><span class="text-xl sm:text-2xl font-black text-slate-800">{currentRecord?.dci ?? '--'}</span><span class="text-[10px] sm:text-xs font-bold text-slate-400 ml-0.5">kcal</span></div></div>
                    </div>
                    <div class="bg-white p-3 sm:p-4 rounded-xl shadow-sm border border-gray-100 border-l-4 border-l-purple-500 transition-transform hover:scale-[1.02] h-full flex flex-col justify-between">
                        <div class="min-h-[24px] flex items-center justify-between mb-2"><span class="text-[10px] sm:text-xs font-black text-slate-700 uppercase tracking-widest truncate">{$t('metrics.metabolic_age')}</span><button on:click={() => openInfo('metabolic_age')} class="text-slate-300 hover:text-indigo-500 transition-colors"><Info size={16} /></button></div>
                        <div class="flex items-end justify-between mt-1"><Clock size={20} class="text-purple-400 mb-1" strokeWidth={2} /><div class="text-right leading-none"><span class="text-xl sm:text-2xl font-black {getStatusColor('meta', currentRecord?.metabolicAge, currentRecord).replace('bg-', 'text-').replace('-100', '-600')}">{currentRecord?.metabolicAge ?? '--'}</span><span class="text-[10px] sm:text-xs font-bold text-slate-400 ml-0.5">{$t('units.years')}</span></div></div>
                    </div>
                </div>
                
                <InfoModal isOpen={showInfoModal} title={infoModalTitle} message={infoModalContent} closeAriaLabel={$t('common.close') || 'Close'} on:close={() => showInfoModal = false} />
            </div>
                  ...
                  {#each chartData.pointsData as p, i}
                    ...
                    <rect role="presentation" x={p.x - 3} y="0" width="6" height="100" fill="transparent" class="cursor-pointer hover:fill-gray-50/10"
                          on:mouseenter={() => {
                            ...
                            hoveredIndex = i;
                            hoveredPointData = { ...p, unitKey: activeChartUnitKey, isHighPoint, alignment };
                          }}
